...
import sys, glob, os, subprocess, shlex, re, string, getopt, MySQLdb

############
# SETTINGS #
############
conf_file='~/orthomclSoftware-v2.0.9/doc/OrthoMCLEngine/Main/orthomcl.config.template' # template config file
...
def usage():
    sys.stderr.write('Use: %s -i <indir> -n <name_for_db> --cpu=<nr_of_cpus> --chunks=<split fasta file in n chunks, same as cpu if not specified>\n'%sys.argv[0])
...
def run_commands_parallel(commands, cpu, chunks):
    ...
    while (commands):
        ...
        for process in xrange(cpu):
	      ...
	      print "Running: ", c
	      args = shlex.split(c)
	      subproc = subprocess.Popen(args, shell=False) #shell should be false, otherwise arguments are not parsed correctly  
...
if __name__=='__main__':
    ...
    if os.path.isfile(lockname):
        sys.stderr.write('Lockfile %s exists, it seems another instance of orthomcl is running on this DB. Please pick another name or remove the lockfile.\n'%lockname) 
        sys.exit(1)
    ...
    try:
        ...
        sys.stderr.write('Database "%s" exists, please run with -d to delete and recreate\n'%name)
    ...
    text=text.replace('dbConnectString=dbi:mysql:orthomcl:3307','dbConnectString=dbi:mysql:%s'%name)   
    ...
    # CHECK FASTA
    fasta_files=glob.glob(indir+'/*.faa')+glob.glob(indir+'/*.fasta')+glob.glob(indir+'/*.fa')
    ...
    for count,filename in enumerate(fasta_files):
        ...
        for line in text:
            if line[0]=='>':
                ...
                output.write('>%s\n'%orf_id)
                output_tags.write(string.join([name_id,orf_id,short_filename,line[1:]],'\t')+'\n')
                orf_count+=1
            else:
                output.write(line+'\n')
        output.close()
